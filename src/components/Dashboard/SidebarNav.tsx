...
const SidebarNav: React.FC<SidebarNavProps> = ({ className }) => {
  ...
  return (
    <nav
      className={cn(
        'flex flex-col h-full bg-sidebar text-sidebar-foreground w-64 p-4 space-y-1 fixed left-0 top-0 bottom-0',
        className
      )}
    >
      <div className="flex items-center mb-6 px-2 pt-2">
        <BriefcaseBusiness className="h-8 w-8 text-primary mr-2" /> 
        <span className="text-2xl font-bold text-foreground">YourLogo</span>
      </div>
